#!/usr/bin/env bash
# Runs Terraform against Proxmox with the API token (secrets/proxmox-api-token.sops.yaml)
# and the B2 state-backend key (secrets/b2-state-backend.sops.yaml) injected for the
# duration of this one command — same sops-exec-env pattern as
# scripts/etcd-snapshot-backup.sh's B2 credentials. Nothing decrypted ever touches
# disk or shell history. Used both by hand on the jump box and by the GitHub Actions
# workflow (.github/workflows/terraform-proxmox.yml).
#
# Usage: ./tf.sh plan
#        ./tf.sh apply
set -euo pipefail
...
STATE_SECRETS_FILE="${SCRIPT_DIR}/secrets/b2-state-backend.sops.yaml"
...
if command -v flock >/dev/null 2>&1; then
  ...
  LOCK_FILE="${LOCK_DIR}/homelab-terraform-proxmox.lock"
